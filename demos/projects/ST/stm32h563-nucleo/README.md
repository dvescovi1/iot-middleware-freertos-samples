# Connect a STMicroelectronics STM32H563ZI-NUCLEO using Azure IoT middleware for FreeRTOS

## What you need

- ST Microelectronics DevKit:
- Ethernet Cable
...
To build the device image, navigate to the `iot-middleware-freertos-samples` directory and run the following commands:
  ...
  ```bash
    cmake -G Ninja -DVENDOR=ST -DBOARD=stm32h563-nucleo -Bstm32h563-nucleo .
    cmake --build stm32h563-nucleo
  ```

After the build completes, confirm that a folder named `stm32h563-nucleo/` was created and it contains a file named `demo/projects/ST/stm32h563-nucleo/iot-middleware-sample.bin`. 
...
3. In File Explorer, find the STM Devkit that's connected to your computer. The device appears as a drive on your system with the drive label `DIS_H563`.
